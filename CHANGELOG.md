...
- **Container List caching in `dockerProvider`**: Introduced a short-lived (2s TTL) in-memory cache for Docker/Podman container list queries in `dockerProvider.FindByPorts`, preventing N+1 Unix socket HTTP requests during repeated queries or watch ticks.
