...
- **Eliminated N+1 process info query overhead in port searches and listings**: Added `InfoBasicBatch` to `ProcessInspector` and updated `processInfos` in `internal/inspector/search.go` to batch process metadata lookups. On Darwin, reusing a single `libproc` session across all PIDs avoids repeated `dlopen`/`dlclose` and FFI symbol registrations.
